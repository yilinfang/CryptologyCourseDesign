...
{
public:
	spn_plus(unsigned char* key0);
	void Encrypt(unsigned char* plaintext);
private:
	unsigned char sbox[256 + 1] =
	{
	...
	unsigned char pbox_inverse[128 + 1];
	unsigned char key[11 * 4 * 4 + 1];
	unsigned char * Xor(unsigned char * a, unsigned char* b); //a = a ^ b
	unsigned char * Sub(unsigned char* input, unsigned char* sbox);
	unsigned char * Per(unsigned char* input, unsigned char* pbox);
	void RotWord(unsigned char* input);
	void KeyExpansion(unsigned char* key0);
};
